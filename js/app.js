...
const App = (() => {
  ...
  const CATEGORY_ICONS = {
    'Frais': '🧊', 'Épicerie': '🛒', 'Bébé': '👶',
    'Hygiène': '🧴', 'Surgelés': '❄️', 'Boissons': '🥤', 'Autre': '📦'
  };
  ...
  async function saveList() {
    ...
    try {
      const obj = {};
      state.list.forEach(i => { obj[i.id] = i; });
  ...
  async function saveHistory() {
    const obj = {};
    state.history.forEach(s => { obj[s.id] = s; });
  ...
  async function searchOFF() {
    ...
    if (!query) { showToast('Saisis un nom de produit', 'error'); return; }
    ...
    try {
      const res = await fetch(
        `https://world.openfoodfacts.org/api/v2/search?search_terms=${encodeURIComponent(query)}&fields=product_name_fr,product_name,brands&page_size=6&lc=fr`,
        { headers: { 'User-Agent': "Listo - Personal shopping list app" } }
      );
      const data = await res.json();
      const products = (data.products || []).filter(p => p.product_name_fr || p.product_name);

      if (products.length === 0) {
        resultsDiv.innerHTML = '<div style="color:var(--text3);font-size:12px;padding:8px 0;">Aucun résultat — essaie un autre terme ou ajoute manuellement.</div>';
        return;
      }
      ...
      resultsDiv.innerHTML = '<div style="color:#C0392B;font-size:12px;padding:8px 0;">Open Food Facts indisponible — utilise la saisie manuelle ci-dessous.</div>';
    }
  }
