const App = (() => {
  ...
  const CAT_ICONS = {'Frais':'🧊','Épicerie':'🛒','Bébé':'👶','Hygiène':'🧴','Surgelés':'❄️','Boissons':'🥤','Boulangerie':'🥖','Autre':'📦'};
  ...
  function openListPicker() {
    ...
    setTimeout(() => {
      ...
      if (inp) inp.addEventListener('keydown', e => { if (e.key === 'Enter') submitNewList(); });
    }, 100);
  }
  ...
  function renderFavorites() {
    ...
    bar.innerHTML = `<div class="fav-label">⭐ Fréquents</div>
      <div class="fav-chips">${sorted.map(f =>
  ...
  function renderAutocomplete(query) {
    ...
    Object.values(state.favorites).forEach(f => {
      const norm = f.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  ...
  function addItem({ name, emoji, category, qty, note }) {
    ...
    list.items[id] = { id, name, emoji, category, qty, note, done: false, addedBy: state.user.initial, addedAt: Date.now() };
  ...
  function finishShopping() {
    ...
    list.items = {};
    remaining.forEach(i => { list.items[i.id] = { ...i, done: false }; });

    saveCurrentList();
    const histObj = {};
    state.history.forEach(s => { histObj[s.id] = s; });
    DB.setHistory(histObj);

    renderList(); renderHistory(); updateHeader();
  ...
  // ── PUBLIC ──
  return {
    init, setUser, switchScreen,
    openListPicker, switchList, deleteList, submitNewList,
    toggleItem, changeQty, deleteItem,
    openEditItem, saveEditItem,
    quickAdd, addAdvanced, addFavorite,
    selectAutocomplete, addFromHistory, recreateList,
    finishShopping, closeModal,
    showToast,
  };
